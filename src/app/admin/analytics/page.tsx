    ...
    <div className="space-y-3">
      {data.map(item => (
        <div key={item.label} className="flex items-center gap-3">
          <span className="text-xs text-slate-500 w-24 shrink-0 truncate text-right">{item.label}</span>
    ...
    <div className="flex flex-col sm:flex-row items-center gap-6">
      ...
      <div className="space-y-2">
        {[
          ...
          { label: 'Other',    value: total - active - pending - rejected, color: 'bg-slate-200' },
        ].map(item => (
          <div key={item.label} className="flex items-center gap-2">
            <div className={`w-2.5 h-2.5 rounded-full ${item.color}`} />
            <span className="text-xs text-slate-500">{item.label}</span>
            <span className="text-xs font-medium text-slate-700 ml-auto pl-4">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map(kpi => (
          <div key={kpi.label} className="bg-white rounded-2xl border border-slate-100 p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-semibold text-slate-900">{kpi.value.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          <h2 className="text-sm font-semibold text-slate-800 mb-5">Listing status breakdown</h2>
          {data ? (
            <DonutChart
            ...
            <div className="h-32 flex items-center justify-center text-sm text-slate-300">Loading...</div>
          )}
        </div>

        {/* Top cities */}
        <div className="bg-white rounded-2xl border border-slate-100 p-5">
          <h2 className="text-sm font-semibold text-slate-800 mb-5">Active listings by city</h2>
          {cityData.length > 0
            ? <BarChart data={cityData} maxVal={maxCity} />
            : <div className="h-32 flex items-center justify-center text-sm text-slate-300">No data yet</div>}
        </div>
      </div>

      {/* Metrics summary */}
      <div className="bg-white rounded-2xl border border-slate-100 p-5">
        <h2 className="text-sm font-semibold text-slate-800 mb-4">Key metrics summary</h2>
